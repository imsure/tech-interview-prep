...
// Sort the array and binary search on the range of pair distances (0 to max distance among pairs)
// time: O(NlogN * logM) where N is the size of input and M is the max distance among all pairs.
...
public:
  int smallestDistancePair(vector<int>& nums, int k) {
    ...
    while (lo_dist < hi_dist) {
      ...
      if (pair_count >= k) hi_dist = mid_dist; // not mid_dist + 1 coz mid_dist is possibly the answer
      else lo_dist = mid_dist + 1; // must advance lo_dist to mid_dist+1
    }
...
public:
  int smallestDistancePair(vector<int>& nums, int k) {
    ...
    int lo = 0, hi = nums[n-1] - nums[0], mid;
    while (lo < hi) {
      mid = (lo + hi) / 2;
      int left = 0, count = 0;
      for (int right = 0; right < n; ++right) { // two pointers
        while (nums[right] - nums[left] > mid) ++left; // advance left as needed
        count += right - left;
      }
      if (count >= k) hi = mid;
      else lo = mid + 1;
