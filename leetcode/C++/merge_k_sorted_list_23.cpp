...
// time: O(kN), where N is the total # of nodes in all k lists.
// mergeTwoLists takes O(n/k) time and we need to do this by k times,
// so the total would be SUM(1<=i<=k-1)(i*N/k + N/k) = O(kN)
// space: O(1)
...
// Divide and Conquer: merge each pair in the k lists to produce k/2 lists;
// then merge each pair in this k/2 lists; keep doing so until all lists have
// been merged to one list.
...
public:
  ...
  ListNode* mergeKLists(vector<ListNode*>& lists) {
    ...
    int i = 0, j = n - 1;
    while (i < j) {
      new_lists.push_back(mergeTwoLists(lists[i], lists[j]));
      ++i; --j;
    }

    if (i == j) new_lists.push_back(lists[i]);
