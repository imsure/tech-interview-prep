class Solution:
    def loudAndRich(self, richer, quiet):
        ...
        """
        poor2rich = {}
        # for p in range(0, len(quiet)):
        #     poor2rich.setdefault(p, [p])

        for item in richer:
            poor = item[1]
            rich = item[0]
            poor2rich.setdefault(poor, []).append(rich)
            # poor2rich[poor].append(rich)

        print(poor2rich)

        ans = [0] * len(quiet)
        for p in range(0, len(quiet)):
            if p not in poor2rich:
                ans[p] = p
                continue
            ...
            while True:
                next_patch = []
                for key in keys:
                    riches = poor2rich[key]
                    for rich in riches:
                        if rich in poor2rich:
                            next_patch.append(rich)
                        if quiet[rich] < quiteness:
                            less_quite = rich
                            quiteness = quiet[rich]
